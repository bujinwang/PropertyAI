...
describe('DatabaseOptimizationService', () => {
    ...
    beforeEach(() => {
        ...
        mockRedis = new ioredis_1.Redis();
        service = new databaseOptimization_service_1.DatabaseOptimizationService(mockPrisma, 'redis://localhost:6379');
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('analyzePostgresSlowQueries', () => {
        it('should analyze PostgreSQL slow queries', async () => {
            ...
            expect(mockPrisma.$executeRawUnsafe).toHaveBeenCalledWith(expect.stringContaining('CREATE EXTENSION IF NOT EXISTS pg_stat_statements'));
    ...
    describe('createMissingIndexes', () => {
        it('should suggest missing indexes without executing when execute=false', async () => {
            ...
            expect(mockPrisma.$executeRawUnsafe).not.toHaveBeenCalledWith(expect.stringContaining('CREATE INDEX'));
        });
        it('should create indexes when execute=true', async () => {
            ...
            expect(mockPrisma.$executeRawUnsafe).toHaveBeenCalledWith(expect.stringContaining('CREATE INDEX'));
        });
    });
    ...
    describe('analyzeQueryPlans', () => {
        it('should analyze query plans', async () => {
            ...
            expect(result).toHaveLength(1);
            expect(result[0].analysis).toContainEqual(expect.objectContaining({
    ...
    describe('runHealthCheck', () => {
        it('should return health check data for PostgreSQL and MongoDB', async () => {
            ...
            mongoose_1.default.connection.db.command
...
//# sourceMappingURL=databaseOptimization.service.test.js.map