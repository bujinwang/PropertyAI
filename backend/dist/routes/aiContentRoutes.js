...
router.get('/', async (req, res) => {
    try {
        ...
        const aiContents = await mongoModels_1.AiContent.find(filter)
            .sort({ createdAt: -1 });
        res.json({
            status: 'success',
...
router.get('/:id', async (req, res) => {
    try {
        const aiContent = await mongoModels_1.AiContent.findOne({ contentId: req.params.id });
        if (!aiContent) {
...
router.post('/generate', async (req, res) => {
    try {
        const { contentType, prompt, relatedEntityId, relatedEntityType, modelName, modelVersion, parameters, tags, userId } = req.body;
        ...
        const newAIContent = new mongoModels_1.AiContent({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        // Update the content
        const updatedAIContent = await mongoModels_1.AiContent.findOneAndUpdate({ contentId: req.params.id }, { $set: req.body }, { new: true, runValidators: true });
        res.json({
            status: 'success',
