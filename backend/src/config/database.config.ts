...
export const buildConnectionUrl = (): string => {
  const { host, port, database, user, password } = postgresConfig.connection;
  const sslParam = postgresConfig.connection.ssl ? '?sslmode=require' : '';
  return `postgresql://${user}:${password}@${host}:${port}/${database}${sslParam}`;
};

// Database URL for Prisma
export const getDatabaseUrl = (): string => {
  return process.env.DATABASE_URL || buildConnectionUrl();
};

// Generate PostgreSQL role creation SQL
export const getRoleCreationSQL = (): string => {
...
-- Grant privileges
GRANT ALL PRIVILEGES ON DATABASE ${postgresConfig.connection.database} TO propertyai_admin;
GRANT CONNECT ON DATABASE ${postgresConfig.connection.database} TO propertyai_app;
GRANT CONNECT ON DATABASE ${postgresConfig.connection.database} TO propertyai_readonly;

-- Connect to the database and set up schema permissions
\\c ${postgresConfig.connection.database}
