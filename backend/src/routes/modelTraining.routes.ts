...
router.post('/jobs', protect, async (req, res) => {
  try {
    ...
    const baseModel = modelRegistryService.getModel(baseModelId);
    if (!baseModel) {
    ...
    // Start training job
    const jobId = await modelTrainingService.startTrainingJob({
      baseModelId,
      datasetId,
      hyperparameters,
      outputModelName,
...
router.get('/jobs', protect, (req, res) => {
  try {
    const jobs = modelTrainingService.listJobs();
...
router.get('/jobs/:jobId', protect, (req, res) => {
  try {
    ...
    const job = modelTrainingService.getJobStatus(jobId);
    ...
    logger.error(`Error getting training job ${req.params.jobId}: ${error}`);
    res.status(500).json({
      status: 'error',
 ...
 * @access  Private
 */
router.delete('/jobs/:jobId', protect, (req, res) => {
  try {
    const { jobId } = req.params;
    const success = modelTrainingService.cancelJob(jobId);
    ...
    logger.error(`Error cancelling training job ${req.params.jobId}: ${error}`);
    res.status(500).json({
      status: 'error',
