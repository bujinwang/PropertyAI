...
export class DatabaseOptimizationService {
  ...
  constructor(prisma: PrismaClient, redisUrl: string) {
    ...
    this.queryLogPath = path.join(logsDir, 'slow-queries.json');
    this.optimizationHistoryPath = path.join(logsDir, 'optimization-history.json');
  }
  
  /**
   * Analyze PostgreSQL slow queries and provide optimization recommendations
   */
  public async analyzePostgresSlowQueries(): Promise<any[]> {
    try {
      ...
      logger.error('Error analyzing PostgreSQL slow queries: ' + (error?.message || error));
      throw error;
    }
  }
   ...
   */
  public async analyzeMongoSlowQueries(): Promise<any[]> {
    try {
      logger.info('Analyzing MongoDB slow queries');
      ...
      logger.error('Error analyzing MongoDB slow queries: ' + (error?.message || error));
      throw error;
    }
  }
   ...
   */
  private async getQueryOptimizationRecommendations(query: string): Promise<string[]> {
    try {
      ...
      logger.error('Error getting query optimization recommendations: ' + (error?.message || error));
   ...
   */
  private getMongoQueryOptimizationRecommendations(queryProfile: any): string[] {
    const recommendations: string[] = [];
   ...
   */
  private logSlowQueries(dbType: 'postgres' | 'mongodb', queries: any[]): void {
    try {
      ...
      if (fs.existsSync(this.queryLogPath)) {
        try {
          const logsContent = fs.readFileSync(this.queryLogPath, 'utf8');
          logs = JSON.parse(logsContent);
        } catch (err) {
          logger.warn('Error reading query logs, starting fresh');
        }
      }
      ...
      // Write back to file
      fs.writeFileSync(this.queryLogPath, JSON.stringify(logs, null, 2));
    } catch (error: any) {
      logger.error('Error logging slow queries: ' + (error?.message || error));
    }
  }
   ...
   */
  public async createMissingIndexes(execute: boolean = false): Promise<string[]> {
    try {
      logger.info('Analyzing and creating missing indexes');
      ...
      logger.error('Error creating missing indexes: ' + (error?.message || error));
      throw error;
    }
  }
  ...
  public async optimizeConnections(): Promise<void> {
    try {
      ...
      if (mongoose.connection.db) {
        const mongoStatus = await mongoose.connection.db.command({ serverStatus: 1 });
        const mongoConnections = mongoStatus.connections;
        
        this.logOptimizationAction('mongo_connection_analysis', { mongoConnections });
      }
    } catch (error: any) {
      logger.error('Error optimizing connections: ' + (error?.message || error));
      throw error;
    }
  }
   ...
   */
  private logOptimizationAction(action: string, details: any): void {
    try {
      let history = [];
      
      // Read existing history if available
      if (fs.existsSync(this.optimizationHistoryPath)) {
        const fileContent = fs.readFileSync(this.optimizationHistoryPath, 'utf8');
      ...
      fs.writeFileSync(this.optimizationHistoryPath, JSON.stringify(history, null, 2));
    } catch (error: any) {
      logger.error('Error logging optimization action: ' + (error?.message || error));
    }
  }
   ...
   */
  public async runVacuumAnalyze(): Promise<void> {
    try {
      ...
      logger.error('Error running VACUUM ANALYZE: ' + (error?.message || error));
      this.logOptimizationAction('vacuum_analyze', { 
        success: false, 
  ...
  public async getDatabaseMetrics(): Promise<any> {
    try {
      ...
      logger.error('Error getting database metrics: ' + (error?.message || error));
      throw error;
    }
  }
   ...
   */
  public async detectUnusedIndexes(): Promise<any[]> {
    try {
      logger.info('Detecting unused and redundant indexes');
      ...
      if (mongoose.connection.db) {
        ...
        for (const collection of collections) {
          ...
          for (const idx of indexes) {
            ...
            const stats = indexStats.cursor.firstBatch.find((s: any) => s.name === idx.name);
      ...
      logger.error('Error detecting unused indexes: ' + (error?.message || error));
      throw error;
    }
  }
   ...
   */
  public async analyzeQueryPlans(compareWithPrevious: boolean = true): Promise<any[]> {
    try {
      ...
      const plansDir = path.join(__dirname, '../../logs/database/plans');
      ...
      for (const query of slowQueries) {
        try {
          ...
          const planFile = path.join(plansDir, `plan_${queryHash}.json`);
          
          // Get the execution plan
          const explainQuery = `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) ${query.query}`;
          const explainResult = await this.prisma.$queryRawUnsafe(explainQuery) as any[];
          ...
          let regressions = [];
          if (compareWithPrevious && fs.existsSync(planFile)) {
            try {
              const previousPlanData = JSON.parse(fs.readFileSync(planFile, 'utf8'));
              regressions = this.detectPlanRegressions(previousPlanData.plan, plan);
            } catch (err: any) {
              logger.error('Error comparing with previous plan: ' + (err?.message || err));
            }
          }
          
          // Save the current plan
          fs.writeFileSync(planFile, JSON.stringify(currentPlan, null, 2));
          ...
          logger.error(`Error analyzing plan for query: ${err?.message || err}`);
        }
      }
      
      return results;
    } catch (error: any) {
      logger.error('Error analyzing query plans: ' + (error?.message || error));
      throw error;
    }
  }
   ...
   */
  private canExplainQuery(query: string): boolean {
   ...
   */
  private findSequentialScans(node: any, issues: any[]): void {
   ...
   */
  private findHashJoins(node: any, issues: any[]): void {
   ...
   */
  private findExpensiveFilters(node: any, issues: any[]): void {
   ...
   */
  private findIneffcientSorts(node: any, issues: any[]): void {
   ...
   */
  private detectPlanRegressions(oldPlan: any, newPlan: any): any[] {
   ...
   */
  private getPlanNodeTypes(plan: any): string[] {
    const types: string[] = [];
  ...
  public async runHealthCheck(): Promise<any> {
    try {
      ...
      logger.error('Error running database health check: ' + (error?.message || error));
      throw error;
    }
  }
