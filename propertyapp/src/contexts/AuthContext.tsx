...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const normalizeUser = (userData: ExtendedUser): ExtendedUser => ({
    ...userData,
  ...
  const updateProfile = async (userData: Partial<ExtendedUser>): Promise<void> => {
    ...
    try {
      ...
      setUser((currentUser) => 
        currentUser ? normalizeUser({ ...currentUser, ...response.data }) : null
      );
    } catch (error) {
  ...
  const verifyMFA = async (email: string, code: string): Promise<{ token: string; user: ExtendedUser }> => {
    ...
    try {
      ...
      return { ...response, user: normalizeUser(response.user) } as { token: string; user: ExtendedUser };
    } catch (error) {
